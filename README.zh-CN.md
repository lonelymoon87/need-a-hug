...
> 给 coding agent 的轻量情绪急救层：当人已经过载时，先别继续推计划。
...
`need-a-hug` 有意做得比 therapy chatbot 更轻、更窄、更安全。它不是心理健康产品，而是给 coding agent 的 humane UX layer。

| 对比对象 | 通常在做什么 | `need-a-hug` 的做法 |
| --- | --- | --- |
| Therapy chatbot | 模拟长期咨询、情绪追踪、筛查或类似治疗的对话 | 只做短暂的情绪急救，然后温和地回到用户真正要做的事 |
| Productivity coach | 把难受立刻转成目标、习惯、清单或行动计划 | 先暂停生产力压力，等用户稳一点再说下一步 |
| 泛泛的 system prompt | 加一些“要有同理心”的宽泛要求，工具调用一多就容易被挤掉 | 给出具体触发词、第一句形状、安全边界和回到任务的规则 |
| 普通 coding agent | 人已经过载时还在继续计划、改文件、解释 | 停止硬推，降低认知负担，只恢复一个很小、可验证的下一步 |

## 当 Agent 把事情弄得更糟时

一个很重要的场景是：让人崩溃的不是任务本身，而是 assistant 自己。

```text
Agent 改了太多文件，测试全挂了，我现在感觉这个项目快失控了。
```

这时候正确答案不是再来一个大计划。Agent 应该先承认这真的很烦，停下大范围操作，检查刚才发生了什么，然后一次只恢复一个可验证的小事实。

```text
我们先都慢下来。

我不会继续盲目改文件。先看 diff，确认最小的坏点，然后只做一个恢复步骤，验证完再继续。
```
